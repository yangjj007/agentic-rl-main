...
def main():
    ...
    skip_final_save = os.environ.get("DYME_SKIP_FINAL_SAVE", "").strip().lower() in {"1", "true", "yes"}
    if skip_final_save:
        if accelerator.is_main_process:
            print("[DyME] DYME_SKIP_FINAL_SAVE=1; skipping final checkpoint save.", flush=True)
        return
