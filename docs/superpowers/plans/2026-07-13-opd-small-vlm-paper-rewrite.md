...
- [ ] Run `rg -n "first VLM OPD|首个 VLM OPD|sub-1B|互补" docs/paper_reconstruction` and verify no unsupported first claim remains.
...
- [ ] Expand Small VLM and multimodal CoT coverage with LLaVA, TinyLLaVA, MobileVLM, SmolVLM, Multimodal-CoT, LLaVA-CoT, Insight-V, Mulberry, and SFT-or-RL.
- [ ] Expand VLM RLVR coverage with DeepSeekMath/GRPO background, DeepSeek-R1, Visual-RFT, VLM-R1, Reason-RFT, LMM-R1, MM-Eureka, Vision-R1, R1-VL, and OpenVLThinker.
- [ ] Expand SFT/RL hybrid coverage with DyME, LUFFY, CHORD, SRFT, and KDRL.
- [ ] Expand KD/OPD coverage with KD, sequence KD, MiniLLM, GKD, VOLD, Decomposed OPD, VA-OPD, IW-OPD, RG-OPD, DOPD, TA-OPD, PW-OPSD, SFD, and GateKD.
- [ ] Expand curriculum coverage with TSCL, ALP-GMM, Auto-CEI, and self-evolving reasoning curriculum where relevant.
- [ ] Ensure each paragraph states what prior work solved and the exact remaining gap; avoid bibliography dumping.
...
- [ ] Count unique citation keys used in the draft and require at least 35.
- [ ] Verify every cited key exists in `references_seed.bib`.
- [ ] Search for forbidden claims: `first VLM OPD`, `完全无 gold`, `已超过 60`, and unqualified `no-gold`.
- [ ] Check that the abstract can be summarized without implementation terminology in one sentence.
- [ ] Check that Introduction presents the method before any controller implementation details.
- [ ] Check that all unfinished effectiveness claims remain marked running or missing.
