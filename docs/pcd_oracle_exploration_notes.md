...
- 失败形态：full-CoT 大幅下降，但 eval log 出现大量短模板污染，例如 `Reasoning style` 和异常 `Answer:` 空行格式。

结论：

`student_hint_short` 证明“缩短 teacher-SFT repair target”有一定价值：它显著降低 full-CoT 污染，但没有单独解决后期 GRPO 有效信号不足。后续不再把它作为唯一改动冲 `0.60+`；而是把它作为低污染 repair target，与 OPD decay/cap 或动态采样组合。
...
- eval 输出不重新出现 full-CoT / privileged tag 污染。
