...
from opsd_utils.hard_target_metrics import (
    OnlineSftSourceCounts,
    no_full_hint_hard_sft_enabled,
)
...
from opsd_utils.opsd_loss import (
    ...
    slice_student_completion_logits,
)
...
class DyMETrainer(Trainer):
    ...
    def _apply_teacher_probe_routing(
        ...
        *,
        ...
        failure_route = str(
            (self.opsd_config.get("gate") or {}).get("teacher_probe_failure_route", "sft")
            or "sft"
        ).lower()
        ...
        def failure_mode_for(row: int) -> int:
            has_correct_i, _, _, _, _ = group_metadata_for(row)
            return teacher_probe_failure_mode(
                group_has_correct=bool(has_correct_i),
                route=failure_route,
            )

        def failure_route_name(mode: int, reason: str) -> str:
            prefix = {MODE_GRPO: "grpo", MODE_SKIP: "skip", MODE_SFT: "sft"}[mode]
            ...
            if probe_cfg["skip_no_evidence"] and not evidence_status["evidence_present"]:
                ...
                append_teacher_probe_record(
                    ...
                    record=build_teacher_probe_record(
                        ...
                        teacher_output="",
                        score=0.0,
                        final_route=failure_route_name(fallback_mode, "no_evidence"),
                        answer_flag=answer_flag,
                        prompt_profile=str(probe_cfg.get("prompt_profile", "")),
                        harness=str(probe_cfg.get("harness", "")),
                        harness_version=str(probe_cfg.get("harness_version", "")),
                        max_new_tokens=int(probe_cfg.get("max_new_tokens", 0) or 0),
        ...
        if max_per_batch > 0 and len(candidate_indices) > max_per_batch:
            ...
            for idx in skipped:
                completion_modes[idx] = failure_mode_for(idx)
    ...
    def _should_force_sft_replace(
        ...
        if gate_cfg.get("disable_force_sft_replace") or no_full_hint_hard_sft_enabled(
            gate_cfg
        ):
            return False
    ...
    def _generate_and_score_completions(
        ...
        mixed_group_hard_replay_used_count = 0
        mixed_group_all_wrong_skip_count = 0
        mixed_group_hard_replay_targets = {}
        ...
        if opsd_active:
            ...
            if bool((self.opsd_config.get("gate") or {}).get("mixed_group_hard_replay", False)):
                hard_replay_plan = build_mixed_group_hard_replay_plan(
                    completion_ids=completion_ids,
                    completion_mask=completion_mask,
                    acc_rewards=acc_rewards,
                    num_generations=self.num_generations,
                ...
                mixed_group_hard_replay_targets = hard_replay_plan.targets
                mixed_group_all_wrong_skip_count = len(hard_replay_plan.all_wrong_indices)
                for skip_idx in hard_replay_plan.all_wrong_indices:
                    if skip_idx < len(completion_modes):
                        completion_modes[skip_idx] = MODE_SKIP
                opsd_debug.log(
                    "opsd_router",
                    "mixed-group shortest-correct hard-replay plan",
                    target_count=len(mixed_group_hard_replay_targets),
                    mixed_wrong_count=hard_replay_plan.mixed_wrong_count,
                    all_wrong_skip_count=mixed_group_all_wrong_skip_count,
                    correct_source_indices=hard_replay_plan.correct_source_indices,
                )
            else:
                mixed_group_hard_replay_targets = {}
            prompt_modes = [
        ...
        online_sft_source_counts = OnlineSftSourceCounts()
        ...
        for i in range(len(sft_padded_ids)):
            ...
            elif i in mixed_group_hard_replay_targets:
                target_ids, target_mask = mixed_group_hard_replay_targets[i]
                target_ids = target_ids.to(device=device, dtype=completion_ids.dtype)
                target_mask = target_mask.to(device=device, dtype=completion_mask.dtype)
                completion_id_ = torch.cat([target_ids, completion_ids[i][0:0]])
                completion_mask_ = torch.cat([target_mask, completion_mask[i][0:0]])
                advantange_ = torch.ones(
                    completion_mask_.size(0),
                    device=device,
                    dtype=torch.float,
                )
                opsd_mask_list.append(False)
                sft_replaced = True
                mixed_group_hard_replay_used_count += 1
            elif sft_check[i] or (opsd_active and use_sft) or force_sft_replace:
                if sft_check[i]:
                    online_sft_source_counts.record("slot")
                elif opsd_active and use_sft:
                    online_sft_source_counts.record("route")
                else:
                    online_sft_source_counts.record("forced")
        ...
        if self._health_monitor is not None:
            ...
            online_sft_rates = online_sft_source_counts.as_rates(
                total_completions=local_routing_n
            )
            gate_cfg = self.opsd_config.get("gate") or {}
            no_full_hint_hard_sft_mode = no_full_hint_hard_sft_enabled(gate_cfg)
            self._health_monitor.record_routing(
                global_step,
                {
                    "sft_replaced_ratio": sum(sft_replaced_list) / local_routing_n,
                    "legacy_online_sft_slot_rate": online_sft_rates[
                        "routing/legacy_online_sft_slot_rate"
                    ],
                    "legacy_online_sft_route_rate": online_sft_rates[
                        "routing/legacy_online_sft_route_rate"
                    ],
                    "legacy_online_sft_forced_rate": online_sft_rates[
                        "routing/legacy_online_sft_forced_rate"
                    ],
                    "legacy_online_sft_rate": online_sft_rates[
                        "routing/legacy_online_sft_rate"
                    ],
                    "full_hint_hard_target_rate": online_sft_rates[
                        "routing/full_hint_hard_target_rate"
                    ],
                    "no_full_hint_hard_sft_enabled": (
                        1.0 if no_full_hint_hard_sft_mode else 0.0
                    ),
                    "opsd_skipped_degenerate": opsd_skipped_degenerate,
                    ...
                    "mixed_group_hard_replay_rate": mixed_group_hard_replay_used_count / local_routing_n,
                    "mixed_group_all_wrong_skip_rate": mixed_group_all_wrong_skip_count / local_routing_n,
    ...
    def _compute_loss(self, model, inputs):
        ...
        opsd_debug.set_detail_step(global_step)
        raw_grpo_loss = loss
        grpo_loss_tensor = raw_grpo_loss.detach()
        ...
        if self.opsd_config.get("enabled", False) and inputs.get("opsd_mask") is not None:
            ...
            loss = _combine_grpo_opsd_losses(
                raw_grpo_loss,
                grpo_weight=grpo_weight,
                opsd_loss=None,
                opsd_weight=opsd_effective_weight,
            )
            ...
            if global_opsd_count > 0 and local_opsd_count > 0:
                opsd_debug.log(
                    ...
                    grpo_weight=grpo_weight,
                    grpo_loss=float(raw_grpo_loss.detach().item()),
                    local_opsd_count=local_opsd_count,
                )
                ...
                with opsd_debug.timed("opsd_loss", "compute_vlm_opsd_loss_masked_batch"):
                    ...
                    opsd_loss = compute_vlm_opsd_loss_masked_batch(
                        ...
                        loss_type=opsd_loss_type,
                        srkl_alpha=srkl_alpha,
                        token_weighting=opsd_loss_cfg.get("token_weighting"),
                    )
                opsd_debug.hang_probe(
            ...
            teacher_traj_scheduled_weight = effective_teacher_traj_weight(
            ...
            if adaptive_weights is not None:
                teacher_traj_scheduled_weight = adaptive_weights[1]
            self._metrics[mode].setdefault("loss/teacher_traj_scheduled_weight", []).append(
                teacher_traj_scheduled_weight
            )
            teacher_traj_effective_weight = 0.0
            if traj_cfg["enabled"] and inputs.get("teacher_traj_mask") is not None:
                ...
                teacher_traj_effective_weight = effective_teacher_traj_exposure_weight(
                    scheduled_weight=teacher_traj_scheduled_weight,
                    enabled=traj_cfg["enabled"],
