#!/usr/bin/env bash
# 200-step RLSD short run for anti-leakage validation.
# Training params: config/config_rlsd_shortrun.yaml
set -euo pipefail

cd "$(dirname "$0")/.."
source "$(dirname "$0")/launch_utils.sh"

CONFIG_PATH="config/config_rlsd_shortrun.yaml"
...
LOG_FILE="$(train_log_path train_rlsd_shortrun)"
