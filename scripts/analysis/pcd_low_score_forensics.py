...
def collect_eval_rows(label: str, run_dir: Path, extra_summaries: list[Path] | None = None) -> list[dict[str, Any]]:
    ...
    for summary in summaries:
        ...
        for row in csv.DictReader(summary.open(encoding="utf-8")):
            ...
            elif raw_label.startswith("eval_"):
                checkpoint = raw_label.removeprefix("eval_").split("_2026", 1)[0]
            out = {
                "run": label,
    ...
    for log in sorted((run_dir / "eval_chartqa").glob("eval_*.log")) + sorted(
        ...
        raw = log.stem
        match = re.search(r"(checkpoint-\d+|final_checkpoint)", raw)
        checkpoint = match.group(1) if match else raw
        rows.setdefault(
            checkpoint,
            {
                "run": label,
    ...
    return sorted(rows.values(), key=lambda row: _checkpoint_sort_key(str(row.get("checkpoint", ""))))
