...
| **DyME** | `train_dyme.sh` | default `config/config.py`; optional `config/config_dyme_deepspeed.py` | `main.py --mode rl` | 动态 SFT/GRPO，无 OPD，4 epoch；memory-tuned config 可选 |
...
| Gate 步数估算 | 600/epoch heuristic | `DYME_FAST_EST_STEPS_PER_EPOCH` | 仅用于 gate 缩放，不等于真实 optimizer steps |
...
# DyME / OPD 仍 OOM 时进一步缩 batch 或生成长度。注意这些改动不再是 matched comparator。
...
| 全量 | scripts/test/ 快速版 |
|------|---------------------|
| `config/config.py` (10 epoch) | `scripts/test/config/config.py` (4 epoch, default DyME comparator config) |
