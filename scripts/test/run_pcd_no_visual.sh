...
if [[ "${VARIANT}" == "deplot_no_vs_opd_pcd_gold_hidden_vold_cold_start" ||
      "${VARIANT}" == "deplot_no_vs_opd_pcd_gold_hidden_ssopd_mixed_group" ]]; then
  echo "retired near-neighbor variant: ${VARIANT}; run only an honestly named matched implementation" >&2
  exit 2
fi
if [[ "${VARIANT}" == "deplot_no_vs_opd_pcd_oracle_hint_opd_no_full_hint_hard_sft_adaptive_supervision" ]]; then
...
SAVE_STEPS_DEFAULT=""
SAVE_TOTAL_LIMIT_DEFAULT=""
case "${VARIANT}" in
  deplot_no_vs_opd_pcd_oracle_hint_opd_no_full_hint_hard_sft_adaptive_supervision|deplot_no_vs_opd_pcd_gold_hidden_*)
    SAVE_STRATEGY_DEFAULT="steps"
    SAVE_STEPS_DEFAULT="50"
    SAVE_TOTAL_LIMIT_DEFAULT="3"
    ;;
esac
SAVE_STRATEGY="${DYME_SAVE_STRATEGY:-${SAVE_STRATEGY_DEFAULT}}"
SAVE_STEPS="${DYME_SAVE_STEPS:-${SAVE_STEPS_DEFAULT}}"
SAVE_TOTAL_LIMIT="${DYME_SAVE_TOTAL_LIMIT:-${SAVE_TOTAL_LIMIT_DEFAULT}}"
...
OPSD_MODE="${DYME_OPSD_MODE:-dyme_teacher_probe_opd}"
TEACHER_PROBE="${DYME_TEACHER_PROBE:-1}"
TEACHER_PROBE_ALL_WRONG_AFTER_STEP="${DYME_TEACHER_PROBE_ALL_WRONG_AFTER_STEP:-0}"
...
if [[ -n "${DYME_SSOPD_MIXED_GROUP+x}" ]]; then
  echo "DYME_SSOPD_MIXED_GROUP is retired; use DYME_MIXED_GROUP_HARD_REPLAY" >&2
  exit 2
fi
...
DISABLE_ONLINE_SFT_SLOTS="${DYME_DISABLE_ONLINE_SFT_SLOTS:-0}"
ONLINE_SFT_ON_ALL_WRONG="${DYME_ONLINE_SFT_ON_ALL_WRONG:-1}"
TEACHER_PROBE_FAILURE_ROUTE="${DYME_TEACHER_PROBE_FAILURE_ROUTE:-sft}"
...
if [[ "${VARIANT}" == "deplot_no_vs_opd_pcd_oracle_hint_opd_no_full_hint_hard_sft_adaptive_supervision" ]]; then
  ...
  TEACHER_CORRECT_REPAIR_MODE=opd
  OPSD_SKIP_DEGENERATE=0
  DISABLE_ONLINE_SFT_SLOTS=1
  ONLINE_SFT_ON_ALL_WRONG=0
...
case "${VARIANT}" in
  deplot_no_vs_opd_pcd_gold_hidden_*)
  TEACHER_PROVIDERS="format_only,visual_facts_deplot"
  TEACHER_PROBE_PROMPT_PROFILE="${DYME_TEACHER_PROBE_PROMPT_PROFILE:-chartqa_short_answer}"
  TEACHER_PROBE_MAX_NEW_TOKENS="${DYME_TEACHER_PROBE_MAX_NEW_TOKENS:-96}"
  ORACLE_GOLD_SUFFIX_EXPECTED=0
  ...
  OPSD_WEIGHT_DECAY=0
  TEACHER_TRAJ_WEIGHT_DECAY=0
  OPSD_MAX_PER_PROMPT=8
  DYNAMIC_TRIGGER_MONITOR=0
  EVAL_FORMAT_REWARD=0
  CHART_COT_VERIFY=1
  CHART_COT_GATE_MODE=gate
  ADAPTIVE_SUPERVISION=0
  GLOBAL_SIGNAL_LOGGING=1
    ;;
esac
if [[ "${VARIANT}" == "deplot_no_vs_opd_pcd_gold_hidden_no_opd" ]]; then
  OPSD_WEIGHT=0.0
  GRPO_WEIGHT=1.0
  ADAPTIVE_SUPERVISION=0
fi
if [[ "${VARIANT}" == "deplot_no_vs_opd_pcd_gold_hidden_grpo_only" ]]; then
  OPSD_MODE=dyme
  TEACHER_MODEL=""
  TEACHER_PROBE=0
  ...
  GRPO_WEIGHT=1.0
  OPSD_MAX_PER_PROMPT=0
  EFFECTIVE_SAMPLING=0
  CHART_COT_VERIFY=0
fi
if [[ "${VARIANT}" == "deplot_no_vs_opd_pcd_gold_hidden_fallback_only" ]]; then
  OPSD_MODE=dyme
  TEACHER_MODEL=""
  TEACHER_PROBE=0
  TEACHER_PROBE_ALL_WRONG_AFTER_STEP=-1
  OPSD_WEIGHT=0.0
  GRPO_WEIGHT=0.0
  DISABLE_ONLINE_SFT_SLOTS=0
  ONLINE_SFT_ON_ALL_WRONG=1
  OPSD_MAX_PER_PROMPT=0
  EFFECTIVE_SAMPLING=0
  CHART_COT_VERIFY=0
fi
if [[ "${VARIANT}" == "deplot_no_vs_opd_pcd_gold_hidden_uncond_opd_no_full_hint_hard_sft" ]]; then
  TEACHER_PROBE=0
  OPSD_MODE=dyme_teacher_probe_opd
  ADAPTIVE_SUPERVISION=0
fi
if [[ "${VARIANT}" == "deplot_no_vs_opd_pcd_gold_hidden_opd_only_no_full_hint_hard_sft" ]]; then
  GRPO_WEIGHT=0.0
  ADAPTIVE_SUPERVISION=0
fi
...
if [[ "${VARIANT}" == "deplot_no_vs_opd_pcd_gold_hidden_opd_no_full_hint_hard_sft_adaptive_target020" ]]; then
  ADAPTIVE_TARGET_READINESS=0.20
fi
if [[ "${VARIANT}" == "deplot_no_vs_opd_pcd_gold_hidden_token_reliability_clrc" ]]; then
  OPSD_MAX_PER_PROMPT=0
  ADAPTIVE_SUPERVISION=1
  ADAPTIVE_READINESS_SOURCE=global_grpo_route
  ADAPTIVE_TARGET_READINESS=0.20
...
if [[ "${VARIANT}" == "deplot_no_vs_opd_pcd_gold_hidden_mixed_group_shortest_correct_hard_replay" ]]; then
  ...
  EFFECTIVE_SAMPLING=0
  CHART_COT_VERIFY=0
  MIXED_GROUP_HARD_REPLAY=1
fi

# Adaptive supervision owns effective-sampling activation. Keep the legacy
# step/progress boundaries only for non-adaptive variants.
...
TRAIN_ENV=(
  ...
  "DYME_SKIP_FINAL_SAVE=${DYME_SKIP_FINAL_SAVE:-0}"
  "DYME_OPSD_MODE=${OPSD_MODE}"
  ...
  "DYME_DISABLE_ONLINE_SFT_SLOTS=${DISABLE_ONLINE_SFT_SLOTS}"
  "DYME_ONLINE_SFT_ON_ALL_WRONG=${ONLINE_SFT_ON_ALL_WRONG}"
  "DYME_TEACHER_PROBE_FAILURE_ROUTE=${TEACHER_PROBE_FAILURE_ROUTE}"
  ...
  "DYME_MIXED_GROUP_HARD_REPLAY=${MIXED_GROUP_HARD_REPLAY}"
...
if [[ -n "${SAVE_STEPS}" ]]; then
  TRAIN_ENV+=("DYME_SAVE_STEPS=${SAVE_STEPS}")
fi
if [[ -n "${SAVE_TOTAL_LIMIT}" ]]; then
  TRAIN_ENV+=("DYME_SAVE_TOTAL_LIMIT=${SAVE_TOTAL_LIMIT}")
fi
if [[ -n "${MAX_STEPS}" ]]; then
