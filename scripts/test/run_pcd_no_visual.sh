...
ONLINE_SFT_TARGET_STYLE="${DYME_ONLINE_SFT_TARGET_STYLE:-}"
SFT_COLD_START_STEPS="${DYME_SFT_COLD_START_STEPS:-0}"
...
if [[ "${VARIANT}" == "deplot_no_vs_opd_pcd_gold_hidden_opd_no_full_hint_hard_sft_adaptive_supervision_sft_repair" ]]; then
  ...
  TEACHER_SFT_TARGET_STYLE="${DYME_TEACHER_SFT_TARGET_STYLE:-answer_only}"
  ONLINE_SFT_TARGET_STYLE="${DYME_ONLINE_SFT_TARGET_STYLE:-answer_only}"
  SFT_COLD_START_STEPS="${DYME_SFT_COLD_START_STEPS:-100}"
  TEACHER_SFT_TARGET_MAX_TOKENS="${DYME_TEACHER_SFT_TARGET_MAX_TOKENS:-64}"
  TEACHER_PROBE_MAX_NEW_TOKENS="${DYME_TEACHER_PROBE_MAX_NEW_TOKENS:-320}"
...
echo "online SFT target: style=${ONLINE_SFT_TARGET_STYLE:-default}"
echo "SFT cold start: steps=${SFT_COLD_START_STEPS}"
...
if [[ -n "${ONLINE_SFT_TARGET_STYLE}" ]]; then
  TRAIN_ENV+=("DYME_ONLINE_SFT_TARGET_STYLE=${ONLINE_SFT_TARGET_STYLE}")
fi
if [[ "${SFT_COLD_START_STEPS}" != "0" ]]; then
  TRAIN_ENV+=("DYME_SFT_COLD_START_STEPS=${SFT_COLD_START_STEPS}")
fi
if [[ -n "${MAX_STEPS}" ]]; then
