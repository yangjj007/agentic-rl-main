...
def effective_teacher_traj_exposure_weight(
    *,
    scheduled_weight: float,
    enabled: bool,
    global_traj_count: int,
) -> float:
    """Return the teacher-trajectory weight actually exposed this step."""
    if not enabled or int(global_traj_count) <= 0:
        return 0.0
    return float(scheduled_weight)
