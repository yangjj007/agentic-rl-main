...
def test_finish_step_returns_online_sft_source_metrics() -> None:
    ...
    hm.record_routing(
        2,
        {
            "legacy_online_sft_slot_rate": 0.125,
            "legacy_online_sft_route_rate": 0.25,
            "legacy_online_sft_forced_rate": 0.0,
            "legacy_online_sft_rate": 0.375,
            "full_hint_hard_target_rate": 0.375,
        },
    )

    metrics = hm.finish_step(2)

    assert metrics["routing/legacy_online_sft_slot_rate"] == 0.125
    assert metrics["routing/legacy_online_sft_route_rate"] == 0.25
    assert metrics["routing/legacy_online_sft_forced_rate"] == 0.0
    assert metrics["routing/legacy_online_sft_rate"] == 0.375
    assert metrics["routing/full_hint_hard_target_rate"] == 0.375


def test_health_monitor_alerts_on_no_hard_sft_invariant_violation() -> None:
    hm = TrainingHealthMonitor({"enabled": True, "log_alerts_immediately": False})
    hm.reset_step(3)
    hm.record_routing(
        3,
        {
            "no_full_hint_hard_sft_enabled": 1.0,
            "legacy_online_sft_rate": 0.03125,
            "full_hint_hard_target_rate": 0.03125,
        },
    )

    assert ALERT_HARD_TARGET_INVARIANT in hm._step_alerts
...
def test_finish_step_returns_mixed_group_hard_replay_metrics() -> None:
    hm = TrainingHealthMonitor({"enabled": True, "metrics_every_step": True, "log_every_step": False})
    hm.reset_step(7)
    hm.record_routing(
        7,
        {
            "mixed_group_hard_replay_rate": 0.25,
            "mixed_group_all_wrong_skip_rate": 0.50,
        },
    )

    metrics = hm.finish_step(7)

    assert metrics["routing/mixed_group_hard_replay_rate"] == 0.25
    assert metrics["routing/mixed_group_all_wrong_skip_rate"] == 0.50
