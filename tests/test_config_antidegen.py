...
def test_probe_config_no_full_hint_hard_sft_env(monkeypatch):
    import config.config_opd_7b_dyme_probe as probe_mod

    monkeypatch.setenv("DYME_DISABLE_ONLINE_SFT_SLOTS", "1")
    monkeypatch.setenv("DYME_ONLINE_SFT_ON_ALL_WRONG", "0")
    monkeypatch.setenv(
        "DYME_TEACHER_PROBE_FAILURE_ROUTE",
        "mixed_grpo_all_wrong_skip",
    )
    mod = importlib.reload(probe_mod)

    gate = mod.DYME_OPSD_CONFIG["gate"]
    assert gate["disable_online_sft_slots"] is True
    assert gate["online_sft_on_all_wrong"] is False
    assert gate["teacher_probe_failure_route"] == "mixed_grpo_all_wrong_skip"


def test_probe_config_step_checkpoint_env(monkeypatch):
    import config.config_opd_7b_dyme_probe as probe_mod
    ...
    assert dyme_args["save_strategy"] == "steps"
    assert dyme_args["save_steps"] == 50
    assert dyme_args["save_total_limit"] == 3
