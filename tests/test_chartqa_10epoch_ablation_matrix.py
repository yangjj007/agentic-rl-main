...
def test_chartqa_10epoch_matrix_dry_run_lists_full_default_matrix(tmp_path: Path) -> None:
    env = {
        **os.environ,
    ...
    assert "scripts/test/run_dyme_matched_4epoch.sh --variant pure --epochs 10" in out
    ...
    assert "Variant: oracle_official_best_4e" in out
    assert "bash scripts/test/run_pcd_no_visual.sh 4 --resume none --variant deplot_no_vs_opd_pcd_oracle_hint" in out
    assert "deplot_no_vs_opd_pcd_gold_hidden_grpo_only" in out
    assert "deplot_no_vs_opd_pcd_gold_hidden_opd_no_full_hint_hard_sft_adaptive_supervision" in out
    assert "deplot_no_vs_opd_pcd_gold_hidden_token_reliability_clrc" in out
    assert "deplot_no_vs_opd_pcd_gold_hidden_mixed_group_shortest_correct_hard_replay" in out
...
def test_chartqa_10epoch_matrix_supports_shards_and_smoke(tmp_path: Path) -> None:
    ...
    assert "DYME_DYME_MAX_STEPS=2" in out
    assert "DYME_SKIP_FINAL_SAVE=1" in out
    assert "DYME_SAVE_STRATEGY=no" in out
    assert "DYME_DYME_SAVE_STRATEGY=no" in out
